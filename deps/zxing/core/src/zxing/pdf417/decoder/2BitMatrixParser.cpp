...
BitMatrixParser::BitMatrixParser(Ref<BitMatrix> bitMatrix)
 : bitMatrix_(bitMatrix), tryHarder_(0)
{
  rows_ = 0;
  leftColumnECData_ = 0;
   ...
   */
ArrayRef<int> BitMatrixParser::readCodewords(DecodeHints const &hints)
{
  ...
  bool rowInProgress = true; /* earlier: false - if consecutive scans have to be checked */
  int rowNumber = 0;
  int rowHeight = 0;
  for (i = 0; i < height; i++) {
    ...
    next = processRow(rowNumber,i,codewords,next,height);
      // Reinitialize the row counters.
    rowNumber++;
    rowHeight++;
  }

  /* 2012-10-12 hfn remove try/catch NoErrorException eraseCount_=0 */
  erasures_ = trimArray(erasures_, eraseCount_);

  return trimArray(codewords, next);
}

int BitMatrixParser::processRow(int rowNumber, int rowHeight,
        ArrayRef<int> codewords, int next, int height)
{
  ...
  for (int i = 0; i < width; i += MODULES_IN_SYMBOL) {
    ...
    if (columnNumber > 0) {
      ...
      if (tryHarder_ != 0 && cw < 0) {
        symbol = 0;
        for (int mask = MODULES_IN_SYMBOL - 1; mask >= 0; mask--) {
          if (bitMatrix_->get(i + (MODULES_IN_SYMBOL - 1 - mask), rowNumber + height))
          {
            symbol |= 1L << mask;
          }
        }
        cw = getCodeword(symbol,&cw_block);
...
int BitMatrixParser::getCodeword(int64_t symbol, int *pi)
{
  ...
  if (i == -1) {
    ...
    int64_t cw = CODEWORD_TABLE_[i] - 1;
    if(pi!= NULL)
      *pi = cw / 929;
    cw %= 929;
    return (int) cw;
...
{
  bool bRet=
    IsEqual(aLeftColumnTriple_[0],aRightColumnTriple_[1],rownumber)
...
bool BitMatrixParser::IsEqual(int &a, int &b, int rownumber)
{
  ...
  if(!ret) {
...
#if (defined _WIN32 && defined DEBUG)
      {
        WCHAR _sz[64]; swprintf(_sz,L"rownumber=%d, bad  outer column %d\n",
          rownumber,a);OutputDebugString(_sz);
      }
#endif
      a = -1;
    }
    if(b < row30 || b > row59) {
      ...
      {
        WCHAR _sz[64]; swprintf(_sz,L"rownumber=%d, bad  outer column %d\n",
          rownumber,b);OutputDebugString(_sz);
      }
#endif
      b = -1;
    }
